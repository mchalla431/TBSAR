...
void CANSetMsgRemote(CAN_msg *msg);  
 ...
 *----------------------------------------------------------------------------*/
void CANReadMsg (uint32_t can_msgObj, CAN_msg *msg)  
{
...
void CAN_IRQHandler (void)  
{
	...
	switch (can_int) 
	{
		...
		if ((can_msgObj >=  1) &&                 
          (can_msgObj <= 32)   ) 
		{                /*   valid msgObj number */
			CANReadMsg (can_msgObj, &CAN_RxMsg);      /*  read the message  */
		}
		else 
...
void Can_Init(void)
{
	CANSetup();                                   /* setup CAN Controller   */
  
	CANStart();                                   /* start CAN Controller    */
	CANWaitReady ();                              /* wait til tx mbx is empty */
}	
...
void Can_Message_Send(uint32_t id, uint8_t *dbuf, uint8_t len,  uint8_t format)
{
	...
	CAN_TxMsg.id     = id;                        /* initialize message to send */
  
	for (index = 0; index < len; index++)
	{
		CAN_TxMsg.data[index] = dbuf[index];
	}
	...
	if (CAN_TxRdy)
	{                              /* tx message on CAN ConStroller */
      CAN_TxRdy = 0;

      CANWriteMsg (&CAN_TxMsg);    /* transmit message */
	}
}	

void CANSendMsgRemote(uint32_t id, uint8_t *dbuf, uint8_t len,  uint8_t format)
{
  int index;
	
	CAN_RtMsg.id     = id;                        /* initialize message to send */
  
	for (index = 0; index < len; index++)
	{
		CAN_RtMsg.data[index] = dbuf[index];
	}
	
	CAN_RtMsg.len    = len;
	CAN_RtMsg.format = format;
	CAN_RtMsg.type   = REMOTE_FRAME;

	if (CAN_TxRdy)
	{                              /* tx message on CAN Controller */
      CAN_TxRdy = 0;

      CANSetMsgRemote(&CAN_RtMsg);                     /* transmit message */
	}
}	
...
void Can_Message_Receive(CAN_msg *msg)
{
	...
	{                              
		...
		msg->id     	= CAN_RxMsg.id;
		msg->format 	= CAN_RxMsg.format;
		msg->len		= CAN_RxMsg.len;
		msg->type		= CAN_RxMsg.type;
			
		for (index = 0; index < msg->len; index++)
		{
			msg->data[index] = CAN_RxMsg.data[index];
		}	
	}
}	
